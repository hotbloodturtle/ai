...
`~/.claude/skills/android-qa-agent/SKILL.md`에 스킬 파일을 생성하면 Claude Code에서 `/android-qa-agent`로 호출할 수 있습니다. 이 레포의 `configs/skills/android-qa-agent/SKILL.md`를 직접 복사해 수동으로 등록하면 됩니다.
...
> 이 패턴들은 `configs/settings.json`에 이미 포함되어 있으므로, 필요한 경우 해당 파일을 열어 직접 복사해 적용하면 됩니다.
