...
`~/.claude/skills/android-qa-agent/SKILL.md`에 스킬 파일을 생성하면 Claude Code에서 `/android-qa-agent`로 호출할 수 있습니다. 이 레포의 `configs/skills/android-qa-agent/SKILL.md`에 백업이 있으므로 `./restore-configs.sh`로 자동 복원됩니다.
...
> 이 패턴들은 `configs/settings.json`에 이미 포함되어 있으므로 `./restore-configs.sh`로 복원하면 자동 적용됩니다.
