...
AI 에이전트를 위한 마크다운 디자인 시스템 컬렉션. 유명 사이트의 디자인 토큰을 DESIGN.md 형식으로 제공 (upstream 정기 추가됨). **이 기기 로컬 클론 기준 58개** — 아래 사이트 목록은 upstream 전체(증가 중)이며 `git pull`로 동기화한다.
...
- 58개 사이트의 DESIGN.md 보유 (Claude, Vercel, Linear, Notion, Stripe 등 / upstream 정기 추가)
...
## 포함된 사이트 (upstream 전체 목록, 로컬 클론은 58개)
